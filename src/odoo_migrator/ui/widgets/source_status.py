...
class SourceStatus(QWidget):
    ...
    def set_versions(self, versions: list[int], snapshots=None, selections: dict[int, SourceSelection] | None = None) -> None:
        ...
        for version in versions:
            ...
            selection = (selections or {}).get(version); snapshot = (snapshots or {}).get(version) if snapshots else None; card.set_snapshot(snapshot, selection); self.cards.append(card); self.row.addWidget(card); lines.append(f"Odoo {version}  •  {card.status.text()}")
        self.row.addStretch()
        details = []
