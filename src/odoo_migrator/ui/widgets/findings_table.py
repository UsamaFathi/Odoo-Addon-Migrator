...
class FindingsTable(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for label, value in (("All", "All"), ("Blockers", "blocker"), ("Review Required", "review_required"), ("Warnings", "warning")):
        ...
        self.category.currentTextChanged.connect(lambda value: self.model.setFilter(category=value))
    ...
    def setFindings(self, findings) -> None:  # noqa: N802
        ...
        self.step.blockSignals(True); self.addon.blockSignals(True); self.category.blockSignals(True)
        ...
        self.category.clear(); self.category.addItem("All"); self.category.addItems(sorted({f.concern for f in findings}))
        self.step.setCurrentText("All"); self.addon.setCurrentText("All"); self.category.setCurrentText("All")
        self.step.blockSignals(False); self.addon.blockSignals(False); self.category.blockSignals(False)

    def clearFilters(self) -> None:  # noqa: N802
        self.search.clear(); self.severity.setCurrentText("All"); self.step.setCurrentText("All"); self.addon.setCurrentText("All"); self.category.setCurrentText("All")
