...
| `xml.view_root.tree_to_list.17_to_18` | Odoo 17 `odoo/addons/base/models/ir_ui_view.py:163` registers `tree` as the view type and `:1234` recursively processes embedded `tree` relational views; `:1393` validates tree roots. Official `addons/sale/views/sale_order_views.xml:183-191` uses `<tree>`. | Odoo 18 `odoo/addons/base/models/ir_ui_view.py:153` registers `list`, `:1343` recursively processes embedded `list` relational views, and `:1519` validates list roots. The same official sale view XML ID `sale.view_order_tree` uses `<list>` in `addons/sale/views/sale_order_views.xml:178-186`. | Exact source/target files in the pinned commits above. | safe auto-fix for root and embedded `ir.ui.view` architecture elements | Use a structured XML parser to rename only parser-confirmed `<tree>` start/end tags inside an `ir.ui.view` `arch` field. XML bytes outside those tag names remain unchanged. XPath expressions and business semantics are not guessed; affected inherited XPaths remain review findings. |
| `xml.action_view_mode.tree_to_list.17_to_18` | Odoo 17 `odoo/addons/base/models/ir_actions.py:280` defaults `ir.actions.act_window.view_mode` to `tree,form`; `:346-367` defines `VIEW_TYPES` with `tree` for `ir.actions.act_window.view.view_mode`. | Odoo 18 `odoo/addons/base/models/ir_actions.py:312` defaults the parent field to `list,form`; `:398-418` defines `VIEW_TYPES` with `list`. The target compute/constraint logic still consumes comma-separated `view_mode` tokens (`:282-300`). `binding_view_types` remains `list,form` in both snapshots and is not rewritten. | Exact source/target `ir_actions.py` files in the pinned commits above. | safe auto-fix for the exact verified action fields | Parse XML structurally and replace only a comma-separated `tree` token in direct `view_mode` fields of `ir.actions.act_window` and `ir.actions.act_window.view`. Arbitrary text, domains, contexts, labels, QWeb, comments, and unrelated fields remain unchanged. |
...
architecture, including embedded relational view architectures because the
official 17/18 view postprocessors explicitly change their child tag sets from
`tree` to `list`. It does not rewrite QWeb, arbitrary data XML, comments, text,
XPath expressions, action fields, or frontend code.

The separate action rule handles only direct `view_mode` fields on
`ir.actions.act_window` and `ir.actions.act_window.view`. It does not touch
`binding_view_types`, even though that field is present in the same model,
because the pinned source already uses `list` there in both versions.
