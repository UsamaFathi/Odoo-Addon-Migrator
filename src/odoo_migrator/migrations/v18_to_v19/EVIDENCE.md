...
| `python.sql_constraints_to_models_constraint.18_to_19` | Literal `_sql_constraints` declarations are supported by Odoo 18. | Odoo 19's ORM reports `_sql_constraints` as no longer supported and requires `models.Constraint`. | Target `odoo/orm/model_classes.py` warning and official `odoo/upgrade_code/18.1-00-sql-constraint.py` conversion script in the pinned Odoo 19 snapshot. | safe auto-fix for literal, comment-free declarations; REVIEW_REQUIRED otherwise | Structured AST replacement only; dynamic/comment-bearing declarations remain for review. |
