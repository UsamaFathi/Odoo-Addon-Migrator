...
def test_engine_copies_and_updates_manifest(tmp_path: Path):
    ...
    (mod / "__manifest__.py").write_text("{'name': 'Demo', 'version': '14.0.1.2.0', 'depends': ['base']}\n")
    out = tmp_path / "out"
    result = MigrationEngine().migrate(src, out, 14, 15)
    assert (mod / "__manifest__.py").read_text().find("14.0.1.2.0") >= 0
    assert (out / "demo" / "__manifest__.py").read_text().find("15.0.1.2.0") >= 0
    assert len(result.changes) == 1
    assert result.metadata_path == out / ".odoo_migrator_run.json"
    metadata = json.loads(result.metadata_path.read_text(encoding="utf-8"))
    assert metadata["migration_path"] == ["14_to_15"]
...
def test_engine_dry_run_does_not_create_output_or_metadata(tmp_path: Path):
    ...
    (mod / "__manifest__.py").write_text("{'name': 'Demo', 'version': '14.0.1'}\n")
    out = tmp_path / "out"
    result = MigrationEngine().migrate(src, out, 14, 15, dry_run=True)
    assert result.output == src
    assert result.metadata_path is None
    assert not out.exists()
    assert "14.0.1" in (mod / "__manifest__.py").read_text()
