...
def test_indexer_finds_model_fields_methods_and_xmlids(tmp_path: Path):
    ...
    (mod / "views" / "view.xml").write_text(
        '<odoo><record id="x_sale_view" model="ir.ui.view"/>'
        '<record id="group_demo" model="res.groups"/></odoo>'
    )
    ...
    assert "x_demo.group_demo" in idx.group_xml_ids
...
def test_javascript_path_index_covers_files_without_odoo_module_marker(tmp_path: Path):
    ...
    assert index.schema_version == INDEX_SCHEMA_VERSION == 10
