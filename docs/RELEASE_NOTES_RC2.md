...
- Compact source-status cards with per-version **Local Exact Source** or **Use verified snapshot** choices.
...
- Verified-source selection no longer implies an immediate download; downloads happen only when analysis needs a missing cached snapshot.
